...
/**
*Todo: 
*     OK  - ssqCur for preloadImages 
*       
*     OK  - animEl (add sc CSS targeting)
*  BUGGY  - preloadImages: make sure that first/last image of each ssc are loaded with priority to allow corect init.
*  OK     - initCanvasDisplay (prog 0.0001 or 0.9999)
*  BUGGY  - native gsap refresh - add "ScrollTrigger.refresh(true)" 
*  OK     - resize -> throttle -> recalc/refresh  -> render
*  OK     - orientation change -> recalc/refresh -> render
*  LOW    - (starting ssq mid viewport) optional
*  OK     - cleanup console log
*  OK     => free version                     ==> (OK SC Limit + OK Image Limit + OK Page Limit + OK No Animation )
*                                             ==> (OK vanillaJs Sticky) ( OK pages/animEl Display/Hide) (OK Init canvasElements) (passive scroll listeners?)
*    SC: 
*       OK - [scrollsequence margintop="" marginbottom="" id=""] 
*    UI: 
*       OK  - add shortcode somewhere visible in the UI (and in scrollseuqnce list)
*       OK  - markers ( ON/OFF + zIndex over WPadmin - not possible? )
*       OK  - scrub (for each ssq)
*       LOW - ("keyframes" - snap)
*       LOW - front end "edit ssq" buttom (IF ADMIN)
*/
/**
* --------------------------------------------------------- PHASE 1 -----------------------------------------------------------------
...
function initSsqHtml(){ let t0 = performance.now();
  ...
  for (let s = 0; s < ssqInput.scLength; s++) { 
    ssqHtml[s]={
      ...
      spacer:allScEl[s].getElementsByClassName('scrollsequence-spacer')[0], 
    };
    ssqHtml[s].page=[];
...
function renderCanvas(sc,page,image, requestor,info) {   // Check screen size, update canvas size (css + canvas) and render with scale and align
  ...
  if (ssqImg.objArray[sc][page][image].aaaLoaded){
    ...
    // CANVAS - How many pixels the canvas has (=quality)
    ssqHtml[sc].context.canvas.width = canvasWidth*1;
    ssqHtml[sc].context.canvas.height = canvasHeight*1;
  ...
  // STEP 1 - TIMELINE DEFINITIONS 
    for (let s = 0; s < ssqInput.scLength; s++) { 
      ...
      timelineSc[s] = gsap.timeline({
        ...
        scrollTrigger:{
          trigger: ssqHtml[s].sticky,
          start: 'top top', // 'element viewport'
          end:ssqInput.sc[s].scSpacer+"px  top" ,
              ...
              for(var k=0; k<ssqInput.sc[s].page[i].animEl[j].ssqnce_anim_io_out_list.length  ; k++) { // Loop PAGE[i] ELEMENT [j] Anim LIST [k]
                switch(ssqInput.sc[s].page[i].animEl[j].ssqnce_anim_io_out_list[k]._type) {
                  case 'ssqnce_anim_io_fade':
                    ...
                    break;                          
                  case 'ssqnce_anim_io_x':
                    ssqInput.sc[s].page[i].animEl[j].ssqnce_anim_io_out_list[k]._type='x';
                    break;
                  case 'ssqnce_anim_io_y':
                    ssqInput.sc[s].page[i].animEl[j].ssqnce_anim_io_out_list[k]._type='y';
                    break;
                  case 'ssqnce_anim_io_scale':
